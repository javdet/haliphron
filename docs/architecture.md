...
| 5 | Live logs in a multi-cluster setup | The latency is acceptable. We do not proxy `kubectl logs -f`, only chunks relayed through the controller |
| 6 | opencode | Not in v1 |
| 7 | CRD versioning | `v1alpha1` → `v1` through a conversion webhook |
| 8 | Repository cache | Not doing it |
| 9 | Where results and logs live | Two modes behind one port (section 9.2). The default relays them through the controller into the backend, which writes them to a PVC. Object storage is the second mode and an optimisation — **never a required dependency** |
...
The agent pod executes commands generated by a model, in a repository whose contents may contain prompt injection. **The agent pod is untrusted code.** Hence: a separate namespace, no ServiceAccount token, short-lived scoped tokens, no storage credentials in the pod, and the fact that the control plane accepts nothing from the pod directly.
...
The control plane (backend, Postgres, UI) manages. The data plane (controllers, agent pods, git) executes, and holds the results until the control plane has taken them. A control plane failure does not stop running work and does not lose its results: the controller spools what the pod handed it and keeps retrying the forward.
...
**P5. The result is durable in the data plane before the callback.**
The pod does not report completion until its artifacts have been accepted by something that outlives it — the controller's spool by default, or object storage when that mode is configured. The callback is a delivery optimization, not a correctness condition.

What the principle forbids is a paid-for result existing only in the filesystem of a pod about to be deleted. It does not require object storage to be one of the parties, and reading it as though it did is what made a bucket look mandatory.
...
graph TB
    subgraph CP["Control plane (namespace: haliphron)"]
        ...
        vol[("Artifact volume (PVC)<br/>results, logs, artifacts")]
        s3[("S3 / MinIO<br/>optional, in place of the volume")]
    end
    ...
    be --> pg
    be --> vol
    be -.->|when object-store mode is on| s3
    ctrl -->|lease, heartbeat, status, completion| be
    ctrl --> cr
    ctrl --> sec
    cr --> job
    sec --> job
    job -->|completion notice, result, logs, artifacts| ctrl
    ctrl -->|relays the artifacts| be
    job -.->|object-store mode only| s3
```
...
| Artifact volume | A PVC mounted on the backend | Results, logs and artifacts in the default relay mode (section 9.2) |
| S3 / MinIO | — | The same, in object-store mode. Optional |
...
**The prompt travels in the lease and reaches the pod as an environment variable.** It is stored in `runs.prompt` in PostgreSQL: the system of record already holds everything else about the run, and a second store for one column bought nothing but a dependency. The controller writes it into the per-run Secret under the single key `prompt`, and the Job reads that key with `valueFrom.secretKeyRef` — so the value appears neither in the CR, nor in the Job's spec, nor in `kubectl describe`.

**This is not a contradiction of ADR 33.** That rule keeps secret *material* out of the environment, because the agent inherits the pod's environment and we assume it can exfiltrate anything it can read. The prompt is the one value the agent is meant to read: it is the task. Nothing is protected by withholding it from the process whose entire purpose is to act on it. `secretKeyRef` also names one key rather than spraying a Secret with `envFrom`, which is the failure mode ADR 33 actually describes.

**The ceiling is real and is stated rather than discovered.** A Secret is hard-capped at 1 MiB across all of its keys, and the per-run Secret also carries the git token, the LLM key and `mcp.json`. `maxPromptBytes` is therefore **512 KiB**, enforced at admission with a 413 naming the limit — not found later as a failed Secret create on a run that was already accepted. A workflow step whose accumulated context outgrows that is a real case (section 8), and the answer is to summarise upstream output into the step's input rather than to grow the envelope.

The lease carries `promptSHA256` beside the prompt and the entrypoint verifies it. That keeps the one property the old S3 reference bought: the pod can tell that it is executing exactly what the backend supplied.
...
**The honest formulation of the guarantee: at-least-once execution with converging side effects.** Exactly-once is unachievable in this problem. If a double execution does happen, the attempt checkpoint in PostgreSQL, the deterministic branch name and `create-or-update` for the PR converge on a single PR; only the token spend is doubled.
...
sequenceDiagram
    ...
    Pod->>Ctrl: 1. POST /artifacts (result.md, output.json, agent.log)
    Note over Ctrl: spooled to the controller's volume;<br/>not acknowledged until it is written
    Pod->>Ctrl: 2. POST /completion (a Service inside the cluster)
    Ctrl->>BE: 3. POST /cluster-api/v1/ingest/artifacts (retry with backoff)
    BE->>St: 4. write runs/{id}/... to the volume
    Ctrl->>BE: 5. POST /cluster-api/v1/ingest/completion (retry with backoff)
    Note over Ctrl: the Job reached a terminal state
    Ctrl->>BE: 6. POST /cluster-api/v1/ingest/status {phase: Succeeded}
    BE->>BE: 7. the Run is finished (both lifecycle and contents are in)
    Note over BE: if 5 did not arrive but 6 did —<br/>CompletedWithoutResult,<br/>the backend reads runs/{id}/ back from the store
```

In object-store mode steps 1, 3 and 4 collapse into a presigned PUT straight from the pod, and the completion carries references rather than contents. Nothing else moves. That is the point of putting the split behind a port: the callback path, the report ordering and the `CompletedWithoutResult` recovery do not know which mode is in force.

The pod reports to the controller rather than the backend in either mode: the pod needs no egress to the control plane at all — only the controller's Service, the git host, the LLM and MCP, plus the object store when that mode is on.
...
Code artifacts are passed through git rather than through the artifact store — it is natural and gives a history for free.
...
runs(id, tenant_id, parent_run_id, depth, cluster_id,
     ...
     spec jsonb, prompt, prompt_sha256, agent, model, role_name,
     ...
     cost_usd, input_tokens, output_tokens, pr_url, result_summary, result_ref,
...
run_attempts(run_id, lease_epoch, attempt,  -- the key carries the epoch: attempt resets
             ...
             completed_phases,                -- the idempotent-retry checkpoint (9.3)
...
### 9.2 Artifact storage — a port with two modes

Results, logs and artifacts have unbounded size, must outlive the pod, are read both by a human in the UI and by another agent, and need retention. That is a storage problem, not an object-storage problem, and the distinction matters: an installation should not have to stand up MinIO to run its first agent.

So `ArtifactStore` has two implementations, chosen in the chart by `artifacts.mode`:

| Mode | Path the bytes take | When to choose it |
|---|---|---|
| **`relay`** (default) | pod → controller → backend → a PVC on the backend | Every installation by default. No object storage anywhere |
| `object-store` | pod → S3/MinIO directly, by presigned URL | High artifact volume, many clusters, or an existing bucket with lifecycle rules |
...
runs/{run_id}/completion.json      the pod's report, for the CompletedWithoutResult path
...
`runs.result_ref` holds a URI carrying the scheme — `file://runs/01J8.../result.md` or `s3://haliphron/runs/01J8.../result.md` — so a stored row says which store wrote it. An installation that migrates from one mode to the other keeps its old runs readable instead of orphaning them.

**There is no `prompt.txt` and no `state.json`.** Both moved into PostgreSQL (section 7 for the prompt, 9.3 for the checkpoint). What remains in this store is what actually has no ceiling: outputs, logs and artifacts. That is what makes the object store optional — nothing on the path to *starting* a run touches it any more, only the path to finishing one, and the default mode finishes runs without it.

#### Relay mode

The pod POSTs its artifacts to the same controller Service it already posts the completion to, on a separate path. The controller writes them to its own volume before acknowledging, then forwards them to the backend over the Cluster API with the usual retry and backoff; the backend writes them under the key layout above onto its PVC.

This is what makes P5 hold without a bucket. The acknowledgement the pod waits for means "written to a disk that is not yours", and from that moment the pod may exit: the controller owns delivery, and it survives both the pod's deletion and a backend outage.

The costs are real and are accepted deliberately:

- **The backend is in the data path.** Gigabytes of logs pass through it. Mitigated by a cap on total artifact bytes per run (`artifacts.maxBytesPerRun`, 1 GiB by default, rejected at the controller so the transfer is not paid for twice) and by the log chunking that already exists — chunks are relayed as they are produced rather than in one final push.
- **A PVC is `ReadWriteOnce` on most storage classes.** The backend replica that serves artifact writes must then be singular, or the chart must be given an RWX class. Since the rest of the backend scales freely on `SKIP LOCKED`, the honest statement is: `artifacts.mode=relay` with an RWO volume pins the *artifact writer* to one replica, not the whole control plane. The chart exposes this as `artifacts.pvc.accessMode` and refuses to start more than one replica when it is RWO.
- **No lifecycle rules for free.** Retention becomes the backend's own reaper (`HALIPHRON_REAP_INTERVAL`, already present) walking the volume by prefix age. Object-store mode gets ILM or S3 lifecycle rules instead, generated by the chart.

#### Object-store mode

Unchanged from the original design, and still the better answer at volume. The pod receives no storage keys. When issuing a lease the backend mints:

- presigned PUTs to keys known in advance (`output.json`, `result.md`, `completion.json`, `logs/agent.log`), TTL = `timeoutSeconds` × 2;
- presigned POST policies for the `runs/{run_id}/artifacts/` and `runs/{run_id}/logs/chunks/` prefixes.

A compromised pod writes only into its own prefix and cannot read another run's. There are no presigned **GETs** any more — the two objects the pod used to read, `prompt.txt` and `state.json`, are both gone from this store — which removes exit code 21's most common cause and one whole class of "the run failed and the reason was a URL expiry".

**Reissuing the links.** A TTL of `timeoutSeconds × 2` does not survive three infrastructure retries, and then the pod cannot upload the result — a failure that looks like a lost result on a successfully completed agent. Before creating the Job for an attempt past the first, the controller checks the expiry and, if necessary, takes a new bundle through `POST /cluster-api/v1/leases/{runID}/artifacts`, updating the per-run Secret.

Presigned links are secret material: a bearer capability to write into a specific prefix. Their place is in the per-run Secret, not in the CR's `spec` and not in an environment variable that shows up in `kubectl describe`.

**Rejected options:**

| Option | Why not |
|---|---|
| **Object storage as a hard dependency** | It was one, and it bought nothing a PVC does not for a single-cluster installation. Requiring MinIO to run one agent is a tax on every evaluation and every on-prem install |
| An RWX PVC shared between the cluster and the control plane | Requires NFS/CephFS and a network path from the target cluster to the control plane's storage — which is exactly what P2 forbids. The relay works over the Cluster API the controller already holds open |
| `bytea` / large objects in Postgres | Bloats the database and hurts vacuum, backup time and the recovery plan. This is why the prompt has a 512 KiB ceiling and the logs do not go there at all |
| ConfigMap | A hard 1 MiB limit |
| Pod logs through the k8s API only | The logs live as long as the pod does |

**A hybrid with Postgres.** The summary, up to 64 KiB, is duplicated into `runs.result_summary` so that the run list and search in the UI do not hit the artifact store for every row. The full result is always in the store, with `result_ref` in the database.

### 9.3 The attempt checkpoint and the duplicate guard

The idempotent-retry checkpoint used to be `runs/{id}/state.json`, written by the pod and read back by the next attempt. It is now a column, and the controller is what watches it.

**Why it moved.** The checkpoint is a fact about the run's progress, and the run's progress already lives in PostgreSQL — `observed_phase`, `observed_rank`, `run_attempts`. Keeping one more copy of it in a second store meant two writers for one piece of state, the thing ADR 2 rejects for workflows, and it forced the object store onto the critical path of *starting* an attempt rather than finishing one.

**The shape.** `run_attempts` carries `completed_phases text[]`: the entrypoint phases the attempt got through. The pod reports a phase transition to the controller on the same Service as everything else; the controller records it in the CR's `.status` — which is its durable local copy, available while the backend is down (P4) — and forwards it to `/ingest/status`, where it lands in the column. Before creating the Job for attempt *n+1*, the controller passes the accumulated list in `HALIPHRON_COMPLETED_PHASES`, and the entrypoint skips what is already done. If the `run` phase is in the list, the retry does not call the LLM: it finishes push, PR, upload and notify.

Two properties fall out of this that the S3 checkpoint did not have. The checkpoint survives the *pod's* prefix being unreadable, since it never lived there. And it is visible: `SELECT completed_phases FROM run_attempts` answers "how far did this get before it died", which previously required fetching an object out of a bucket.

**The duplicate guard.** Preventing two live attempts for one run is a database constraint, not a convention:
...
The controller is the monitor: it takes the row before creating a Job, and a conflict means somebody else's attempt is still open — which is either a zombie controller (answered by the epoch, section 7) or its own duplicate reconcile (answered by doing nothing). Kubernetes already contributes `restartPolicy: Never` and `backoffLimit: 0`, so every attempt is a Job the controller created deliberately; the constraint is what makes that true across controllers as well as within one.

**Raising the epoch closes the open attempt.** Otherwise the index is a trap: a controller that vanished mid-run leaves `finished_at IS NULL` behind, and the cluster the work is reassigned to can never take the row. So every statement that raises `lease_epoch` — ack expiry, revocation, reassignment — finishes the outstanding attempt in the same statement, with `failure_class = 'infra'` and a reason naming the fence. The accounting record survives, which is the point of keeping the epoch in the attempt key; only the claim is released.

This guards against duplicate *attempts*. It does not and cannot guard against a genuine double execution across a fencing boundary — that is what the epoch is for, and section 7's honest formulation still stands: at-least-once execution with converging side effects.
...
3.  fetch           read the prompt from the environment, sha256 check
4.  checkpoint      read HALIPHRON_COMPLETED_PHASES; on a retry, skip what is done
...
10. run             start the agent CLI under a timeout, tee to a log file and chunks upstream
...
13. persist         upload result.md, output.json, the log; report the phase
...
17. finalize        upload the final log and completion.json
...
**The phase names survived the move off object storage, and that is deliberate.** `fetch` and `checkpoint` now read the environment instead of a presigned GET, but they remain phases: they are keys in `phaseTimings`, rows in the UI's timeline and entries in the checkpoint itself. Deleting two of them to save two lines of entrypoint would have been a breaking change to three contracts in exchange for nothing.

On the first attempt `HALIPHRON_COMPLETED_PHASES` is absent, and that is a normal answer rather than a failure — the same non-event the old 404 on `state.json` was, minus the round trip that could fail for unrelated reasons.
...
An init container is justified in one case — **downloading the input artifacts** of previous workflow steps into `/workspace/.haliphron/inputs/`: a separate responsibility with separate credentials (a fetch from the controller, or a presigned GET in object-store mode). It is introduced in phase 3, together with workflows.
...
sequenceDiagram
    ...
    API->>DB: INSERT run (status=Queued, cluster_id, prompt)
    alt async=true
    ...
    API-->>C: the lease: spec, prompt, secrets
    ...
    Note over P: prompt and completed phases arrive as env vars
    P->>Git: clone, checkout base, create target branch
    P->>P: role resolution, MCP preparation, start the agent CLI
    P->>C: log chunks as the work proceeds
    P->>Git: commit, push --force-with-lease, create-or-update PR
    P->>C: POST /artifacts (output.json, result.md, agent.log)
    C->>API: /ingest/artifacts
    API->>St: write runs/{id}/...
    P->>C: POST /completion {runID, exitCode, prURL, usage}
...
| 21 | `infra` | An artifact upload failed (a relay POST, or a presigned PUT) | Yes, up to 3 |
...
- the repeat is idempotent thanks to the attempt checkpoint in PostgreSQL (section 9.3), handed to the next Job as `HALIPHRON_COMPLETED_PHASES`: if the `run` phase is past, the second attempt does not call the LLM but finishes push/PR/upload/notify;
...
GET    /api/v1/runs/{id}/result         streamed from the volume, or a 302 in object-store mode
GET    /api/v1/runs/{id}/logs           reading chunks from the artifact store, with a cursor
...
POST /cluster-api/v1/leases/{runID}/artifacts          reissuing the presigned bundle (object-store mode)
POST /cluster-api/v1/clusters/{id}/heartbeat           liveness, lease renewal, commands, reconciliation
POST /cluster-api/v1/ingest/status                     lifecycle and the phase checkpoint (batched)
POST /cluster-api/v1/ingest/artifacts                  artifact bytes, relayed (relay mode)
...
spec:
  ...
  promptSHA256: "..."                      # the prompt itself is in the Secret, never here
  repo:
  ...
  materials:
    secretName: ar-01j8x4k2zq7yb3m9f0r5w6t8cd-s    # holds the prompt and the credentials
    configMapName: ar-01j8x4k2zq7yb3m9f0r5w6t8cd-c
  callbackURL: http://haliphron-controller.haliphron-agents.svc:8080
status:
  ...
  attempt: 1
  failureClass: none
  result: {ref: "file://runs/01J8X4K2.../completion.json", relayed: true}
  usage: {durationMs: 0, totalCostUSD: "0"}   # the pod's self-declaration
  completedPhases: [init, validate, fetch]    # the checkpoint (9.3), replayed into a retry
...
the prompt, the role's files and — in object-store mode — the presigned bundle into
a Secret and a ConfigMap, and puts only their names into the CR. The invariant
"there is not a single secret value in `spec`" is held not by an admission policy
but by the structural schema: there is no place in the type to put a secret.

The prompt rides in the Secret for the same structural reason, though it is not a
credential: leaving a field for it in `spec` would put customer prompts into
`kubectl get agentrun -o yaml` and into every GitOps diff, and the `promptSHA256`
in `spec` is enough to reason about which task a CR describes.

**`.status.completedPhases` is the one piece of run progress the controller owns
locally.** It is written from the pod's phase reports and read back when the
controller builds the next attempt's Job, so an infrastructure retry is idempotent
while the backend is unreachable (P4). It is a cache of what section 9.3 stores;
PostgreSQL remains the system of record, and a controller that lost its CRs
recovers the list from the lease.
...
```json
{
  ...
  "resultRef": {"key": "runs/01J8X4K2/result.md", "sha256": "...", "uploaded": true},
  "outputRef": {"key": "runs/01J8X4K2/output.json", "sha256": "...", "uploaded": true},
  "logRef":    {"key": "runs/01J8X4K2/logs/agent.log", "uploaded": true},
...
The refs name keys rather than buckets: which store holds them is the installation's business, not the pod's, and a pod that never learns a bucket name cannot leak one. `uploaded` is what the backend checks before believing a ref — in relay mode it is the controller's acknowledgement, in object-store mode the pod's own PUT.

`runID` and `attempt` are deliberately duplicated inside the report (delta R4): the same report sits in the artifact store as `completion.json` and is read from there **without** an envelope when the controller died between the webhook and the forwarding. `failedPhase`, `reason` and `message` are a delta too: without them "Failed, config" in the UI is indistinguishable from any other failure, and the evidence exists exactly once.
...
| Reading other runs' results | The pod got access to the whole store | In relay mode the pod names no prefix at all — the controller stamps the `runID` from the CR, so the key is not the pod's to choose. In object-store mode, presigned links to its own prefix and no S3 credentials |
...
- **egress** is allowed for: DNS, the controller's Service, and by CIDR — the LLM endpoint, the git host, the permitted MCPs. In relay mode that is the whole list: the default installation needs no egress rule for storage at all, because the pod never talks to any. Object-store mode adds the S3 endpoint;
...
|---|---|---|
| `haliphron` | The control plane, once | backend, frontend, PostgreSQL (subchart or BYO), the artifact PVC, ingress, ServiceMonitor. MinIO only if `artifacts.mode=object-store` and no bucket is supplied |
...
`docker-compose`: backend + Postgres + frontend, plus the controller with `DockerLauncher`. Two containers rather than three, now that the default mode writes artifacts to a bind mount; a MinIO profile exists for exercising object-store mode. The controller leases work **by the same protocol** as in production but executes it through Docker. A cluster is not needed for developing anything except reconciliation, and the lease protocol stays under constant load.
...
| 13 | Artifact storage is a port with two modes; the default relays to a PVC | Object storage as a hard dependency | Requiring MinIO to run one agent taxes every evaluation and every on-prem install. Only outputs and logs live there now, and nothing on the path to *starting* a run does |
| 14 | The pod holds no storage credentials in either mode | Keys in a Secret | In relay mode it addresses no store at all and cannot name another run's prefix; in object-store mode a presigned link bounds it to its own |
| 15 | The result is durable in the data plane **before** the callback | The callback carries the result | The callback becomes an optimization rather than a correctness condition. "Durable" is the controller's spool or the object store, not necessarily a bucket |
...
| 17 | Logs in chunks through the artifact port, no `kubectl logs -f` | Proxying a live stream | With a pull model proxying is impossible in principle; chunks work identically for every cluster and outlive the pod |
...
| 19 | Idempotency through a checkpoint in Postgres and a deterministic branch | A checkpoint object in S3; repeating from scratch | Progress is already Postgres's to hold; a second copy meant two writers for one fact and put the object store on the path to *starting* an attempt. It is also now queryable: "how far did this get" is a SELECT |
...
| 30 | The prompt is stored in Postgres and delivered as an env var from the per-run Secret | An object in S3 read by presigned GET | It removes the object store from the path to starting a run. The cost is a real ceiling — `maxPromptBytes` 512 KiB, enforced at admission — paid deliberately in exchange for a system that runs without a bucket. `promptSHA256` keeps the integrity check |
...
| 37 | One live attempt per run is a partial unique index, monitored by the controller | A convention plus `backoffLimit: 0` | `backoffLimit` constrains one Job, not two controllers. A constraint in the system of record is the only place the rule cannot be forgotten. It bounds duplicate *attempts*; duplicate *execution* across a fence is still the epoch's job |
| 38 | The prompt as `secretKeyRef`, not a plain env value or a `spec` field | The prompt in the CR's `spec` | Prompts carry customer context. In `spec` they land in `kubectl get -o yaml` and in every GitOps diff; `secretKeyRef` names one key and keeps the value out of `describe` without the `envFrom` failure mode of ADR 33 |
...
|---|---|---|
| **There is no egress filtering by domain.** An agent that reaches a permitted CIDR will contact any host within it | Hour-long repo-scoped git tokens; no storage credentials; access to the cluster API denied; no ServiceAccount token | A customer requirement with a hard perimeter, or a confirmed incident. Ready options: Cilium FQDN policies or an egress proxy with CONNECT filtering |
| **Relay mode puts the backend in the artifact data path**, and an RWO volume pins the artifact writer to one replica | A per-run byte cap, chunked log relay, and `artifacts.mode=object-store` as a one-value switch for installations that outgrow it | When artifact throughput shows up in backend latency, or when a customer needs more than one artifact-writing replica without an RWX class |
...
A vertical slice through every layer. The Cluster API (register, lease, ack, artifacts, heartbeat, ingest), the controller with `JobLauncher`, the `AgentRun` CRD with a conversion webhook, the agent image for claude-code, the backend with REST and MCP, Postgres, the artifact volume, a minimal UI: a run list, status, logs from chunks, the result, the PR. Roles come from the database, without the repository chain. `DockerLauncher` for local development.

The order within the phase is set by three boundaries. First the contracts are fixed — the [Cluster API](contracts/cluster-api.md), the [CRD Go types](contracts/agentrun-crd.md), the [agent image runtime contract](contracts/agent-runtime.md) — and only then do the three tracks proceed in parallel, each against a fake of the other side. The [store](contracts/run-store.md) is fixed fourth: it has no second party across a network, but the whole backend rests on it, and the phase 1 DDL must exist before the use cases start being written. `FakeBackend`, `FakeController` and `FakeControlPlane` (an artifact sink plus a callback stub, against which the image is verified with a single `docker run`) are part of the contracts' deliverable rather than a test utility belonging to one of the teams: without them parallel development is impossible. `DockerLauncher` keeps the lease protocol under constant load in local development, so a divergence between the sides is found on one's own machine rather than in a customer's cluster.
