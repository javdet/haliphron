...
The controller adds exactly five fields to `RenderedRunSpec`, and all five are
...
| `callbackURL` | a cluster-local value: a Service in the controller's namespace. A **base**, not one endpoint: the pod appends the contract's own paths for the completion, the phase reports and the artifacts |
| `artifactMode` | which half of the artifact port this run uses. The backend states it in the lease's bundle; the controller lifts it into the spec because the pod is told it outright and it is not secret |

**The boundary rule is mechanical: whatever the controller materializes does not
go into `spec`.** Secrets, the prompt, presigned links and role files become
cluster objects, and only the names of those objects ride in the CR.

The prompt is the one that is not a credential and obeys the rule anyway.
Prompts carry customer context: a field for one in `spec` would put it into
`kubectl get agentrun -o yaml` and into every GitOps diff. `spec.promptSHA256`
is enough to reason about which task a CR describes, and the text reaches the
container through `valueFrom.secretKeyRef` on a single key of the per-run
Secret — which keeps it out of the Job's spec as well. The invariant
...
attempt's Job as `HALIPHRON_COMPLETED_PHASES`: a `run` phase already passed does
not call the LLM a second time but finishes push/PR/upload/notify (ADR 19).

That list is **the one piece of run progress the controller owns locally**. It
is written from the pod's phase reports and read back when the next Job is
built, so a retry is idempotent while the backend is unreachable (P4). It
accumulates across attempts and is never reset within an epoch, which is the
point: attempt 2 skips the model precisely because attempt 1 ran it. A new epoch
is a new AgentRun, so new ownership starts from nothing.

It is a cache. `run_attempts.completed_phases` in the control plane is the
system of record, and a controller that lost its CRs recovers the list from the
lease rather than from here.
...
| `env` | the non-secret runtime contract variables as literals, plus `HALIPHRON_PROMPT` from a `secretKeyRef` | the list is `ContractEnv` in `api/run/v1/runtime.go`; the one exception is below |
...
keys are fixed by contract 1 — `prompt`, `git-token`, `llm-api-key`, `mcp.json`,
`callback-token`, and `presigned.json` in object-store mode — and all but the
first are invalid environment variable names. `envFrom` silently skips such
keys, leaving an Event in the namespace: the pod starts with no token, no MCP
configuration and no credentials at all, and the first intelligible message
about it is a clone that failed to authenticate.

**One key does become a variable, through `valueFrom.secretKeyRef`.** The prompt
is not a credential — it is the task, the one value the agent is meant to read,
and nothing is protected by withholding it from the process whose purpose is to
act on it. What a `secretKeyRef` buys over a literal `value` is that the text
stays out of the Job's spec and out of `kubectl describe pod`. What it buys over
`envFrom` is that it names **one key**: the failure mode above is exactly a
Secret sprayed wholesale into an environment, and naming one key is the
difference (ADR 38).

The reference is marked `optional`, deliberately. A Secret written by a
controller that predates the key then produces a pod that starts and fails at
its `validate` phase with a message about a missing prompt, rather than one
stuck in `CreateContainerConfigError` with a message about a Secret key.
...
- [x] the prompt is in the Secret under `prompt`, nowhere in the CR, and reaches the container through a `secretKeyRef` rather than a literal `value`
- [x] a lease whose prompt does not hash to its own `promptSHA256`, or to the spec's → `ack {accepted: false, InvalidSpec}` naming the field, no Job created
- [x] a lease whose materials would not fit in a 1 MiB Secret → a negative ack naming `prompt`, rather than a Secret create that fails after the CR exists
- [x] relay mode: no `presigned.json` is written, and the bundle is not in the Secret at all
- [x] object-store mode: `presigned.json` is written, and a bundle near expiry is reissued before the next attempt
- [x] a pod's phase report lands in `status.completedPhases`, deduplicated and in execution order; a repeat patches nothing
- [x] a phase report authenticated for run A cannot add a phase to run B
- [x] `status.completedPhases` accumulates across attempts and reaches the next Job as `HALIPHRON_COMPLETED_PHASES`
- [x] a relayed artifact is spooled before the pod is acknowledged, and survives a controller restart
- [x] the spooled copy is dropped only after the backend acknowledges it; a backend outage leaves it in place
- [x] an artifact for a run past its per-run budget → 413 to the pod, nothing spooled
- [x] an artifact whose body does not match its digest header → 400, nothing spooled
- [x] `ConditionArtifactsRelayed` is False from the first spooled object and True when the last is forwarded, and the TTL reaper leaves the CR alone while it is False
- [x] an abandoned run's spool is discarded rather than forwarded
...
| C16 | The prompt | an `ObjectRef` in `spec` | `spec.promptSHA256`, the text in the per-run Secret, out as one `secretKeyRef` | prompts carry customer context: in `spec` they land in `kubectl get agentrun -o yaml` and in every GitOps diff. The digest is enough to say which task a CR describes (ADR 38) |
| C17 | `callbackURL` | one endpoint | a base URL | there are three paths now — completion, phase, artifacts — and a CR carrying three URLs that differ in their last segment is three chances to disagree |
| C18 | The retry checkpoint | `runs/{id}/state.json` | `status.completedPhases`, replayed as `HALIPHRON_COMPLETED_PHASES` | the controller's local copy is what makes a retry idempotent while the backend is unreachable (P4); the control plane holds the system of record |
| C19 | Artifacts in relay mode | not a case | `spec.artifactMode`, a controller spool, and `ConditionArtifactsRelayed` | the controller holds the only copy of a result between the pod's upload and the backend's acknowledgement. The condition is what stops the TTL reaper collecting a CR — and with it the run's Job, pod and Secret — while that is still true |
| C20 | `ObjectRef` in the status | carried a `bucket`, and a `state` ref | key, digest, `uploaded`; no `state` | which store holds an object is the installation's business, and the checkpoint is not an object any more. `result.relayed` says which path the bytes took, which is what an operator reading a stuck run needs first |
